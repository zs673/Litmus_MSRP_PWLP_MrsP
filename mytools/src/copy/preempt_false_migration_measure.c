...
void* rt_thread(void *tcontext) {
	...
	/* get the lock */
	if (locking == 7)
		lock_1 = litmus_open_lock(locking, 1, "b",
				init_prio_per_cpu(4, 10, 10, 10, 10));
	else
		lock_1 = litmus_open_lock(locking, 1, "b", 0);
...
int job(struct thread_context *ctx) {
	...
	if (ctx->id < interrupter) {
		...
		if (ctx->id == 1){
			if(scheduling == 2)
				loop_for(ctx->id, 0.0000022*2);
			if(scheduling == 4)
				loop_for(ctx->id, 0.0000020*1);
		}

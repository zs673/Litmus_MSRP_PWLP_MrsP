...
pthread_mutex_t mutex_ht1 = PTHREAD_MUTEX_INITIALIZER;

pthread_cond_t cond1_t2 = PTHREAD_COND_INITIALIZER;
pthread_mutex_t mutex_t2 = PTHREAD_MUTEX_INITIALIZER;
...
struct timespec t2_t3_start_time, t2_wake_up;
	...
	do {
		...
		if (ctx->id < interrupter) {
			...
			if (ctx->id == 2) {
				...
				pthread_cond_wait(&cond1_t2, &mutex_t2);
				pthread_mutex_unlock(&mutex_t2);

				/* t2 will release 1ms after t1 gets the lock*/
				clock_gettime(CLOCK_REALTIME, &t2_wake_up);
				t2exec = ( (double)((t2_t3_start_time.tv_sec * 1000000000 + t2_t3_start_time.tv_nsec) - (t2_wake_up.tv_sec * 1000000000 + t2_wake_up.tv_nsec)) + 1000000 ) / 1000000000;
			...
			if (ctx->id == 1) {
				...
				pthread_mutex_lock(&mutex_ht1);
				pthread_cond_signal(&cond_t2);
				
				/*wake up t2 to queue up*/
				pthread_mutex_lock(&mutex_t2);
				pthread_cond_signal(&cond1_t2);
			}
			
			/* get the time when t1 is going to lock */	
			if (ctx->id == 1)
				clock_gettime(CLOCK_REALTIME, &t2_t3_start_time);
			...
			if (ctx->id == 1){
				...
				/* let t3 run */
				pthread_mutex_unlock(&mutex_ht1);
				exit_np();
			...
			pthread_mutex_lock(&mutex_ht1);
			pthread_cond_wait(&cond_t2, &mutex_ht1);
			pthread_mutex_unlock(&mutex_ht1);

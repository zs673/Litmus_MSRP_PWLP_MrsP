...
struct rt_task {
	...
	unsigned int    original_cpu;

	/*************** MrsP support ***************/
	unsigned int	leaving_priority;

	/* A list of obtained locks nested */
	struct mrsp_semaphore** mrsp_locks;
	struct mrsp_semaphore *requesting_lock;

	volatile int	num_mrsp_locks;

	struct task_struct* helper;
	volatile int	preempted;
	volatile int	migrate_back;

	/* NP section support */
	unsigned int	saved_current_priority;
	long long		migrated_time;
	struct non_preemption_budget_timer *np_timer;

	/* set from user space*/
	unsigned int 	max_nested_degree;
	unsigned int 	enable_help;
	unsigned int	np_len;
	unsigned int	mrsp_task;
	/*************** MrsP support end ***************/



	/*************** MSRP support ***************/
	struct msrp_semaphore** msrp_locks;
	volatile int	num_msrp_locks;
	unsigned int 	msrp_max_nested_degree;
	/*************** MSRP support end ***************/



	/*************** FIFO-P support ***************/
	struct fifop_semaphore** fifop_locks;
	struct fifop_semaphore *fifop_requesting_lock;
	volatile int	num_fifop_locks;
	unsigned int 	fifop_max_nested_degree;
	unsigned long 	fifo_ticket;
	unsigned int 	need_re_request;

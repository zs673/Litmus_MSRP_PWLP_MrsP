...
static inline int fp_prio_remove(struct fp_prio_queue* q, struct task_struct* t, unsigned int index)
{
	int ret;
	BUG_ON(!is_queued(t));

	ret = bheap_delete(fp_ready_order, &q->queue[index], tsk_rt(t)->heap_node);
	if (ret == 0 && likely(bheap_empty(&q->queue[index])))
		fpq_clear(q, index);

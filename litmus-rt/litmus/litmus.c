...
//		printk("holder %d migrated. enter non-preemption budget length %d. current time %llu. \n", t->pid, t->rt_param.task_params.np_len, litmus_clock());
//		TRACE("holder %d migrated. enter non-preemption budget length %d. current time %llu. \n", t->pid, t->rt_param.task_params.np_len, litmus_clock());

		__hrtimer_start_range_ns(&bt->timer, ns_to_ktime(litmus_clock() + t->rt_param.task_params.np_len), 0, HRTIMER_MODE_ABS_PINNED, 0);
...
void cancel_non_preemption_budget_timer(struct task_struct* t) {
	...
	//	printk( "task %d cancel np section at %llu", t->pid, litmus_clock());
...
asmlinkage long sys_set_rt_task_param(pid_t pid, struct rt_task __user * param) {
	...
	if (tp.cls != RT_CLASS_HARD && tp.cls != RT_CLASS_SOFT && tp.cls != RT_CLASS_BEST_EFFORT) {
		printk(KERN_INFO "litmus: real-time task %d rejected "
		"because its class is invalid\n", pid);
		goto out_unlock;
	}
	if (tp.budget_policy != NO_ENFORCEMENT && tp.budget_policy != QUANTUM_ENFORCEMENT && tp.budget_policy != PRECISE_ENFORCEMENT) {
		printk(KERN_INFO "litmus: real-time task %d rejected "
...
long litmus_admit_task(struct task_struct* tsk) {
	...
	get_exec_cost(tsk) > min(get_rt_relative_deadline(tsk), get_rt_period(tsk))) {
		TRACE_TASK(tsk, "litmus admit: invalid task parameters " "(e = %lu, p = %lu, d = %lu)\n", get_exec_cost(tsk), get_rt_period(tsk),
...
void exit_litmus(struct task_struct *dead_tsk) {
	...
	if (tsk_rt(dead_tsk)->ctrl_page) {
		TRACE_TASK(dead_tsk, "freeing ctrl_page %p\n", tsk_rt(dead_tsk)->ctrl_page);
...
static struct sysrq_key_op sysrq_kill_rt_tasks_op = { .handler = sysrq_handle_kill_rt_tasks, .help_msg = "quit-rt-tasks(X)", .action_msg =
	"sent SIGKILL to all LITMUS^RT real-time tasks", };
#endif

extern struct sched_plugin linux_sched_plugin;

static int litmus_shutdown_nb(struct notifier_block *unused1, unsigned long unused2, void *unused3) {
...
static struct notifier_block shutdown_notifier = { .notifier_call = litmus_shutdown_nb, };
 ...
 *  @mode:	timer mode
 */
void hrtimer_start_on(int cpu, struct hrtimer_start_on_info *info, struct hrtimer *timer, ktime_t time, const enum hrtimer_mode mode) {
	info->timer = timer;
	info->time = time;
	info->mode = mode;
	...
	if (cpu == smp_processor_id()) {
		...
		__hrtimer_start_range_ns(info->timer, info->time, 0, info->mode, 0);
	} else {

...
static enum hrtimer_restart on_enforcement_timeout(struct hrtimer *timer) {
	struct enforcement_timer* et = container_of(timer, struct enforcement_timer, timer);
	unsigned long flags;
...
/* assumes called with IRQs off */
static void arm_enforcement_timer(struct enforcement_timer* et, struct task_struct* t) {
	lt_t when_to_fire;
	...
	if (likely(!is_np(t))) {
		...
		__hrtimer_start_range_ns(&et->timer, ns_to_ktime(when_to_fire), 0 /* delta */, HRTIMER_MODE_ABS_PINNED, 0 /* no wakeup */);
		et->armed = 1;
	}
}
...
void litmus_current_budget(lt_t *used_so_far, lt_t *remaining) {
	...
	TRACE_CUR("current_budget: sc:%llu start:%llu lt_t:%llu delta:%lld exec-time:%llu rem:%llu\n", sched_clock_cpu(smp_processor_id()), t->se.exec_start,
		litmus_clock(), delta, tsk_rt(t)->job_params.exec_time, budget_remaining(t));

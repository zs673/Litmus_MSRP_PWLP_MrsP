...
void start_bandwidth_timer(struct hrtimer *period_timer, ktime_t period) {
	...
	for (;;) {
		...
		__hrtimer_start_range_ns(period_timer, soft, delta, HRTIMER_MODE_ABS_PINNED, 0);
	}
}
...
static ssize_t sched_feat_write(struct file *filp, const char __user *ubuf, size_t cnt, loff_t *ppos) {
	char buf[64];
	char *cmp;
	int i;
	struct inode *inode;
...
static const struct file_operations sched_feat_fops = { .open = sched_feat_open, .write = sched_feat_write, .read = seq_read, .llseek = seq_lseek,
...
static int hotplug_hrtick(struct notifier_block *nfb, unsigned long action, void *hcpu) {
...
static int migrate_swap_stop(void *data) {
	...
	unlock: double_rq_unlock(src_rq, dst_rq);
...
int migrate_swap(struct task_struct *cur, struct task_struct *p) {
	...
	arg = (struct migration_swap_arg ) { .src_task = cur, .src_cpu = task_cpu(cur), .dst_task = p, .dst_cpu = task_cpu(p), };
...
static void ttwu_queue(struct task_struct *p, int cpu) {
	 ...
	 * specific.
	 */
	if (!is_realtime(p) && sched_feat(TTWU_QUEUE) && !cpus_share_cache(smp_processor_id(), cpu)) {
...
//		if (p->rt_param.task_params.num_mrsp_locks > 0)
//			printk(KERN_EMERG "try to wake up MrsP task %d. state %d, wake flag %d. \n", p->pid, state, wake_flags);
	}
...
int sched_fork(unsigned long clone_flags, struct task_struct *p) {
	...
	if (unlikely(p->sched_reset_on_fork)) {
		if (task_has_dl_policy(p) || task_has_rt_policy(p) || p->policy == SCHED_LITMUS) {
 ...
 * __setparam_dl().
 */
static int dl_overflow(struct task_struct *p, int policy, const struct sched_attr *attr) {
	...
	if (dl_policy(policy) && !task_has_dl_policy(p) && !__dl_overflow(dl_b, cpus, 0, new_bw)) {
		__dl_add(dl_b, new_bw);
		err = 0;
	} else if (dl_policy(policy) && task_has_dl_policy(p) && !__dl_overflow(dl_b, cpus, p->dl.dl_bw, new_bw)) {
...
void rt_mutex_setprio(struct task_struct *p, int prio) {
	...
	out_unlock: __task_rq_unlock(rq);
}
#endif
...
static bool __checkparam_dl(const struct sched_attr *attr) {
	...
	if ((attr->sched_period != 0 && attr->sched_period < attr->sched_deadline) || attr->sched_deadline < attr->sched_runtime)
		return false;
...
static bool dl_param_changed(struct task_struct *p, const struct sched_attr *attr) {
	...
	if (dl_se->dl_runtime != attr->sched_runtime || dl_se->dl_deadline != attr->sched_deadline || dl_se->dl_period != attr->sched_period
			|| dl_se->flags != attr->sched_flags)
		return true;
...
static int __sched_setscheduler(struct task_struct *p, const struct sched_attr *attr, bool user) {
	...
	if (policy < 0) {
		...
		if (policy != SCHED_DEADLINE && policy != SCHED_FIFO && policy != SCHED_RR && policy != SCHED_NORMAL && policy != SCHED_BATCH
...
static int _sched_setscheduler(struct task_struct *p, int policy, const struct sched_param *param, bool check) {
	struct sched_attr attr = { .sched_policy = policy, .sched_priority = param->sched_priority, .sched_nice = PRIO_TO_NICE(p->static_prio), };
...
long sched_setaffinity(pid_t pid, const struct cpumask *in_mask) {
...
#endif
	again: retval = set_cpus_allowed_ptr(p, new_mask);
	...
	out_free_new_mask: free_cpumask_var(new_mask);
	out_free_cpus_allowed: free_cpumask_var(cpus_allowed);
	out_put_task: put_task_struct(p);
	return retval;
}
...
int __sched yield_to(struct task_struct *p, bool preempt) {
	...
	out_unlock: double_rq_unlock(rq, p_rq);
	out_irq:
...
static const struct sched_class fake_sched_class = { .put_prev_task = put_prev_task_fake, };

static struct task_struct fake_task = {
/*
 * Avoid pull_{rt,dl}_task()
 */
.prio = MAX_PRIO + 1, .sched_class = &fake_sched_class, };
...
static struct ctl_table sd_ctl_dir[] = { { .procname = "sched_domain", .mode = 0555, }, { } };

static struct ctl_table sd_ctl_root[] = { { .procname = "kernel", .mode = 0555, .child = sd_ctl_dir, }, { } };
...
static void set_table_entry(struct ctl_table *entry, const char *procname, void *data, int maxlen, umode_t mode, proc_handler *proc_handler,
		bool load_idx) {
 ...
 */
static struct notifier_block migration_notifier = { .notifier_call = migration_call, .priority = CPU_PRI_MIGRATION, };
...
static int sched_cpu_active(struct notifier_block *nfb, unsigned long action, void *hcpu) {
	switch (action & ~CPU_TASKS_FROZEN) {
	case CPU_STARTING:
...
static int sched_cpu_inactive(struct notifier_block *nfb, unsigned long action, void *hcpu) {
	switch (action & ~CPU_TASKS_FROZEN) {
	case CPU_DOWN_PREPARE:
...
static int sched_domain_debug_one(struct sched_domain *sd, int cpu, int level, struct cpumask *groupmask) {
	...
	do {
		...
		if (!(sd->flags & SD_OVERLAP) && cpumask_intersects(groupmask, sched_group_cpus(group))) {
			printk(KERN_CONT "\n");
...
static int init_rootdomain(struct root_domain *rd) {
	...
	free_rto_mask: free_cpumask_var(rd->rto_mask);
	free_dlo_mask: free_cpumask_var(rd->dlo_mask);
	free_online: free_cpumask_var(rd->online);
	free_span: free_cpumask_var(rd->span);
	out: return -ENOMEM;
}
...
enum s_alloc {
	sa_rootdomain, sa_sd, sa_sd_storage, sa_none,
};
...
static int build_overlap_sched_groups(struct sched_domain *sd, int cpu) {
	...
	for_each_cpu(i, span)
	{
		 ...
		 */
		if ((!groups && cpumask_test_cpu(cpu, sg_span)) || group_balance_cpu(sg) == cpu)
			groups = sg;
	...
	fail: free_sched_groups(first, 0);
...
sd_init(struct sched_domain_topology_level *tl, int cpu) {
	...
	*sd = (struct sched_domain ) { .min_interval = sd_weight, .max_interval = 2 * sd_weight, .busy_factor = 32, .imbalance_pct = 125,

			.cache_nice_tries = 0, .busy_idx = 0, .idle_idx = 0, .newidle_idx = 0, .wake_idx = 0, .forkexec_idx = 0,

			.flags = 1 * SD_LOAD_BALANCE | 1 * SD_BALANCE_NEWIDLE | 1 * SD_BALANCE_EXEC | 1 * SD_BALANCE_FORK | 0 * SD_BALANCE_WAKE
					| 1 * SD_WAKE_AFFINE | 0 * SD_SHARE_CPUCAPACITY | 0 * SD_SHARE_PKG_RESOURCES | 0 * SD_SERIALIZE | 0 * SD_PREFER_SIBLING
					| 0 * SD_NUMA | sd_flags,

			.last_balance = jiffies, .balance_interval = sd_weight, .smt_gain = 0, .max_newidle_lb_cost = 0, .next_decay_max_lb_cost = jiffies,
#ifdef CONFIG_SCHED_DEBUG
					.name = tl->name,
#endif
			};
...
#endif
		{ cpu_cpu_mask, SD_INIT_NAME(DIE) }, { NULL, }, };
...
static void sched_init_numa(void) {
	...
	for (i = 0; i < nr_node_ids; i++) {
		for (j = 0; j < nr_node_ids; j++) {
			for (k = 0; k < nr_node_ids; k++) {
				...
				if (distance > curr_distance && (distance < next_distance || next_distance == curr_distance))
	...
	tl = kzalloc((i + level + 1) * sizeof(struct sched_domain_topology_level), GFP_KERNEL);
	if (!tl)
		return;
	 ...
	 */
	for (j = 0; j < level; i++, j++) {
		tl[i] = (struct sched_domain_topology_level ) { .mask = sd_numa_mask, .sd_flags = cpu_numa_flags, .flags = SDTL_OVERLAP, .numa_level = j,
				SD_INIT_NAME(NUMA) };
	}
 ...
 * are onlined.
 */
static int sched_domains_numa_masks_update(struct notifier_block *nfb, unsigned long action, void *hcpu) {
	int cpu = (long) hcpu;
...
static int build_sched_domains(const struct cpumask *cpu_map, struct sched_domain_attr *attr) {
	...
	ret = 0;
	error: __free_domain_allocs(&d, alloc_state, cpu_map);
	return ret;
}
 ...
 */
static int cpuset_cpu_active(struct notifier_block *nfb, unsigned long action, void *hcpu) {
	switch (action) {
...
static void normalize_task(struct rq *rq, struct task_struct *p) {
	...
	struct sched_attr attr = { .sched_policy = SCHED_NORMAL, };
	int old_prio = p->prio;
	int queued;
...
int sched_rt_handler(struct ctl_table *table, int write, void __user *buffer, size_t *lenp, loff_t *ppos) {
	...
	if (0) {
		undo: sysctl_sched_rt_period = old_period;
...
static int cpu_cgroup_can_attach(struct cgroup_subsys_state *css, struct cgroup_taskset *tset) {
	struct task_struct *task;
...
static void cpu_cgroup_attach(struct cgroup_subsys_state *css, struct cgroup_taskset *tset) {
	struct task_struct *task;
...
static void cpu_cgroup_exit(struct cgroup_subsys_state *css, struct cgroup_subsys_state *old_css, struct task_struct *task) {
	/*
...
struct cgroup_subsys cpu_cgrp_subsys = { .css_alloc = cpu_cgroup_css_alloc, .css_free = cpu_cgroup_css_free, .css_online = cpu_cgroup_css_online,
		.css_offline = cpu_cgroup_css_offline, .fork = cpu_cgroup_fork, .can_attach = cpu_cgroup_can_attach, .attach = cpu_cgroup_attach, .exit =
				cpu_cgroup_exit, .legacy_cftypes = cpu_files, .early_init = 1, };

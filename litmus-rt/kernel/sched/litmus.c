...
litmus_schedule(struct rq *rq, struct task_struct *prev) {
	...
	if (next && task_rq(next) != rq) {
		...
		while (next->rt_param.stack_in_use != NO_CPU) {
			...
			if (!litmus->should_wait_for_stack(next)) {
				...
				TRACE_TASK(next, "plugin gave up waiting for stack\n");
				next = NULL;
			...
			if (from_where != task_rq(next)->cpu) {
				...
				TRACE_TASK(next, "next invalid: task keeps " "shifting around!? " "(%d->%d)\n", from_where, task_rq(next)->cpu);
			...
			if (lt_before(_maybe_deadlock + 1000000000L, litmus_clock())) {
		...
		if (other_rq == task_rq(next) && next->rt_param.stack_in_use == NO_CPU) {
			/* ok, we can grab it */
	...
	if (next && (!is_realtime(next) || !tsk_rt(next)->present)) {
		TRACE_TASK(next, "BAD: next (no longer?) valid\n");
		litmus->next_became_invalid(next);
		litmus_reschedule_local();
		next = NULL;
	}
	...
	out: update_enforcement_timer(next);
...
static void enqueue_task_litmus(struct rq *rq, struct task_struct *p, int flags) {
...
static void dequeue_task_litmus(struct rq *rq, struct task_struct *p, int flags) {
...
static void yield_task_litmus(struct rq *rq) {
	TS_SYSCALL_IN_START;TS_SYSCALL_IN_END;
...
static void prio_changed_litmus(struct rq *rq, struct task_struct *p, int oldprio) {
}
 ...
 * exec, fork, wakeup.
 */
static int select_task_rq_litmus(struct task_struct *p, int cpu, int sd_flag, int flags) {
 ...
 * CPU capacity.
 */
.next = &stop_sched_class, .enqueue_task = enqueue_task_litmus, .dequeue_task = dequeue_task_litmus, .yield_task = yield_task_litmus,

.check_preempt_curr = check_preempt_curr_litmus,

.pick_next_task = pick_next_task_litmus, .put_prev_task = put_prev_task_litmus,

#ifdef CONFIG_SMP
	.select_task_rq = select_task_rq_litmus,
#endif

	.set_curr_task = set_curr_task_litmus, .task_tick = task_tick_litmus,

	.get_rr_interval = get_rr_interval_litmus,

	.prio_changed = prio_changed_litmus, .switched_to = switched_to_litmus,
